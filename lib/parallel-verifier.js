...
exports.verifyEmailsParallel = verifyEmailsParallel;
exports.estimateVerificationTime = estimateVerificationTime;
const net_1 = __importDefault(require("net"));
const email_validator_1 = __importDefault(require("email-validator"));
const dns_cache_1 = require("./dns-cache");
// @ts-ignore
...
async function checkSMTP(email, mxRecords, attempt = 0) {
    ...
    return new Promise((resolve) => {
        const socket = net_1.default.createConnection(25, mxRecords[0].exchange);
        let accepted = false;
...
function isDisposable(domain) {
    return disposable_email_domains_1.default.includes(domain.toLowerCase());
}
...
async function verifyEmail(email, enableCatchAll = true, enableCache = true) {
    const result = {
        email,
        valid: false,
        syntax: false,
    ...
    result.syntax = email_validator_1.default.validate(email);
    if (!result.syntax) {
    ...
    try {
        const mxRecords = await dns_cache_1.dnsCache.getMxRecords(domain);
 ...
 */
async function verifyEmailsParallel(emails, options = {}) {
    const { concurrency = 2000, enableCatchAll = true, enableCache = true, onProgress } = options;
    const results = [];
    const startTime = Date.now();
    let processedCount = 0;
    // Group emails by domain for better cache utilization
    const domainGroups = groupEmailsByDomain(emails);
    const sortedDomains = Array.from(domainGroups.keys()).sort((a, b) => domainGroups.get(b).length - domainGroups.get(a).length);
    ...
    for (let i = 0; i < sortedEmails.length; i += concurrency) {
        ...
        const batchResults = await Promise.all(batch.map(email => verifyEmail(email, enableCatchAll, enableCache)));
 ...
 */
function estimateVerificationTime(emailCount, concurrency = 500) {
...
//# sourceMappingURL=parallel-verifier.js.map